...
def load_data(uploaded_file=None):
    ...
    - لو في ملف مرفوع: يختار طريقة القراءة حسب الامتداد (Excel أو CSV).
    - لو مفيش فايل مرفوع: يحاول يقرأ من فايل افتراضي في نفس المجلد (Excel).
    """
    if uploaded_file is not None:
        ...
        # لو CSV
            ...
            # تقدر تغيّر encoding لو عندك مشكلة في العربي
            return pd.read_csv(uploaded_file)

        else:
            st.error("⚠️ نوع الملف غير مدعوم. من فضلك ارفع ملف Excel أو CSV.")
            st.stop()

    # لو مفيش ملف مرفوع – نحاول نقرأ ملف افتراضي (Excel)
    default_name = "Final Sales from 01-09 To 12-11-2025 all stores.xlsx"
    try:
        return pd.read_excel(default_name)
    except Exception:
        st.error("⚠️ من فضلك ارفع ملف (Excel أو CSV) من الشمال، أو حط ملف Excel جنب الكود بنفس الاسم الافتراضي.")
        st.stop()
...
def main():
    ...
    uploaded_file = st.sidebar.file_uploader(
        "ارفع ملف المبيعات والاستوك (Excel أو CSV)",
        type=["xlsx", "xls", "csv"]
    )
